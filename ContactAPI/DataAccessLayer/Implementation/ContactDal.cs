...
namespace ContactApi.DataAccessLayer.Implementation
{
    public class ContactDal : IContactDal
    {
        private static readonly Serilog.ILogger Logger = Log.ForContext<ContactController>(); // Serilog logger

        private IDataContext _context { get; set; }
        private readonly IConfiguration _configuration;
        public ContactDal(IDataContext context, IConfiguration configuration)
        {
            _context = context;
        ...
        public async Task<ResponseEntity<string>> PostData(ContactEntity modeldata)
        {
            using (var transaction = await _context.Database.BeginTransactionAsync())
            {
                ...
                catch (Exception ex)
                {
                    ...
                    Logger.Error($"Exception Message - {ex.Message} ({ex.InnerException?.Message})", ex);
                    return new ResponseEntity<string>()
                    {
                        Result = null,
                        IsSuccess = false,
